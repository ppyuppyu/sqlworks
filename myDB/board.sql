...
INSERT INTO board(bno, title, writer, content) 
VALUES(seq.NEXTVAL, '공지사항입니다', '관리자', '안녕하세요, 가입인사 드려요');
INSERT INTO board(bno, title, writer, content) 
...
FROM board
WHERE ROWNUM > 0 AND ROWNUM <= 10; -- ROWNUM은 1을 포함해야함

-- 페이지 처리
SELECT *
FROM(SELECT ROWNUM rn, title, content
FROM board)
...
FROM board
WHERE ROWID = 'AAAS1gAABAAAgUJAAA';
